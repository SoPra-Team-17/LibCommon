...
namespace spy::util {
    ...
    GameLogicUtils::getWiredCharacter(const gameplay::State &s, character::Character gettingIP) {
        std::optional<std::shared_ptr<character::Character>> resultChar;
        auto character = std::find_if(s.getCharacters().begin(), s.getCharacters().end(), [&gettingIP](character::Character &c) {
